...
//
//	// Send camera to device.
//	//Camera *cuda_camera = NULL;
//	//float size_camera = 1 * sizeof( Camera );
//	//cudaMalloc( ( void** )&cuda_camera, size_camera );
//	//cudaMemcpy( cuda_camera, camera, size_camera, cudaMemcpyHostToDevice );
//
//
//	// TODO: Call performRaycast() kernel.
//
...
//	//bool intersects = stacklessGPUIntersect( r.origin, r.dir,
...
//	//if ( intersects ) {
//	//	normal.x = ( normal.x < 0.0f ) ? ( normal.x * -1.0f ) : normal.x;
//	//	normal.y = ( normal.y < 0.0f ) ? ( normal.y * -1.0f ) : normal.y;
...
//////////////////////////////////////////////////////
//__device__
//bool stacklessGPUIntersect( const glm::vec3 &ray_o, const glm::vec3 &ray_dir,
...
//{
//	// Check left face.
//	if ( fabs( p.x - node.bbox.min.x ) < KD_TREE_EPSILON ) {
//		return node.neighbor_node_indices[LEFT];     
//	}
//	// Check front face.
//	else if ( fabs( p.z - node.bbox.max.z ) < KD_TREE_EPSILON ) {
//		return node.neighbor_node_indices[FRONT];
//	}
//	// Check right face.
//	else if ( fabs( p.x - node.bbox.max.x ) < KD_TREE_EPSILON ) {
//		return node.neighbor_node_indices[RIGHT];
//	}
//	// Check back face.
//	else if ( fabs( p.z - node.bbox.min.z ) < KD_TREE_EPSILON ) {
//		return node.neighbor_node_indices[BACK];
//	}
//	// Check top face.
//	else if ( fabs( p.y - node.bbox.max.y ) < KD_TREE_EPSILON ) {
//		return node.neighbor_node_indices[TOP];
//	}
//	// Check bottom face.
//	else if ( fabs( p.y - node.bbox.min.y ) < KD_TREE_EPSILON ) {
...
//////////////////////////////////////////////////////
//__device__
//glm::vec3 gpuComputeTriNormall( const glm::vec3 &p1, const glm::vec3 &p2, const glm::vec3 &p3 )
//{
//	glm::vec3 u = p2 - p1;
//	glm::vec3 v = p3 - p1;
//
